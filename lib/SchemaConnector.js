...
module.exports = class SchemaConnector {
  constructor(options = {}) {
    ...
    this[integrationDeletedHandler] = () => {
  ...
  async [handleCallback](body) {
    if (this[enableEventLogging]) {
      console.log(
        `REQUEST ${JSON.stringify(body, null, this[eventLoggingSpace])}`
      );
    }
    ...
    switch (body.headers.interactionType) {
      ...
      case 'stateRefreshRequest':
        ...
        await this[stateRefreshHandler](
          body.authentication.token,
          response,
          body.devices,
          body
      ...
      case 'grantCallbackAccess':
        ...
        if (this[callbackAccessHandler]) {
          if (body.callbackAuthentication.clientId === this[clientId]) {
            ...
            await this[callbackAccessHandler](
              body.authentication.token,
              tokenResponse.callbackAuthentication,
              body.callbackUrls,
              body
            );
          } else {
            response = new STBase(
              ...
              `Client ID ${body.callbackAuthentication.clientId} is invalid`,
      ...
      case 'interactionResult':
        ...
        console.log(
          `INTERACTION ERROR: ${body.originatingInteractionType}: ${body.globalError.errorEnum} ${body.globalError.detail}`
        );
        break;
    ...
    if (this[enableEventLogging]) {
      console.log(
        `RESPONSE ${JSON.stringify(response, null, this[eventLoggingSpace])}`
      );
    }
